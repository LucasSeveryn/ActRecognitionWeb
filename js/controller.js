function Ctrl($scope, $http){
  ...
  $scope.logIn = function() {
    ...
    $http.get(url).success(function(data, status, headers, config) { if(data.length==1){var user = data[0]; $scope.loggedIn=true; $scope.userName=user.name; $scope.userSurname=user.surname; $scope.userId=user.userid;}});
  };
